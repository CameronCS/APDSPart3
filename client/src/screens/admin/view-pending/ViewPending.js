...
export default function ViewPending({ user, setUser }) {
    ...
    useEffect(() => {
        ...
        const __fetch = async () => {
            try {
                const result = await fetch('https://localhost:3001/api/employee/payments/pending', {
                    method: 'GET',
                    headers: {
    ...
    const verifyClaim = async (paymentId) => {
        try {
            console.log(paymentId);
            const post_data = await fetch(`https://localhost:3001/api/employee/payments/${paymentId}/verify`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `bearer ${user.token}`
        ...
        <div className="payments-container">
            ...
            <div>
                {pendingPayments.length > 0 ? (
                        ...
                        <tbody>
                            {pendingPayments.map((pm) => (
                                <tr key={pm._id}>
                                    ...
                                    <td>{pm.status}</td>
                                    <td>
                                        <button className="viewButton" onClick={() => verifyClaim(pm._id)}>Verify</button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
                    ...
                    <h2 className="no-payments">No Pending Payments found!</h2>
                )}
            </div>
        </div>
    );
